...
const PortalEventRow = ({ event, muted = false }: { event: TagEvent; muted?: boolean }) => {
  return (
    <div
      className={cn(
        ...
        <div className="mt-10 space-y-8">
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-relaxed text-tag-muted">
              ...
              or weekend afternoons. Decide on the format: presentation, hands-on workshop,
              open discussion, etc.
            </p>
          </div>
  ...
  return (
    <>
      ...
      <div className="rounded-lg border border-tag-border bg-tag-card">
        ...
        {upcoming.length > 0 ? (
          upcoming.map((event) => <PortalEventRow key={event.id} event={event} />)
        ) : (
        ...
        {showPast &&
          past.map((event) => <PortalEventRow key={event.id} event={event} muted />)}
      </div>
